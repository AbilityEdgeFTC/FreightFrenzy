...
public class AutoRightRedAsync7CyclesUnstable extends LinearOpMode {
    ...
    enum State
    {
        ...
        INTAKE5,
        OPEN_ELEVATOR6,
        WAIT_ELEVATOR_DELAY6,
        INTAKE6,
        OPEN_ELEVATOR7,
        WAIT_ELEVATOR_DELAY7,
        INTAKE7,
        PARK,
        IDLE
    }
        ...
        switch (currentState) {
            ...
            case OPEN_ELEVATOR6:
                if(!drive.isBusy())
                {
                    intake.stop();
                    openElevator();
                    currentState = State.WAIT_ELEVATOR_DELAY6;
                }
                break;
            case WAIT_ELEVATOR_DELAY6:
                if(firstTime)
                {
                    offset = runningFor.seconds();
                    firstTime = false;
                ...
                if((runningFor.seconds() - offset) >= elevatorDelay)
                {
                    ...
                    changeState(State.INTAKE6, goToIntake6);
                }
                break;
            case INTAKE6:
                if(hasFreight)
            ...
            case OPEN_ELEVATOR7:
                if(!drive.isBusy())
                {
                    intake.stop();
                    openElevator();
                    currentState = State.WAIT_ELEVATOR_DELAY7;
                }
                break;
            case WAIT_ELEVATOR_DELAY7:
                if(firstTime)
                {
                    offset = runningFor.seconds();
                    firstTime = false;
